...
class DuplicateTracker:
 ...
 logger.info(f"[FILE] Cargadas {len(sent):,} ventas previamente enviadas")
 return sent
 except Exception as e:
 logger.error(f"Error cargando tracking: {e}")
 return set()
 
 logger.info("[FILE] Nuevo archivo de tracking - sin ventas previas")
 return set()
 ...
 logger.debug(f"[EMOJI] Tracking guardado: {len(self.sent_sales):,} ventas")
 except Exception as e:
 ...
 """
 if df.empty:
 logger.warning(f"[WARNING] DataFrame vacío - nada que marcar")
 return
 ...
 logger.info(f"[OK] Registradas {len(new_keys):,} ventas nuevas como enviadas")
 ...
 if already_sent > 0:
 logger.warning(
 f"[INFO] Filtradas {already_sent:,} ventas ya enviadas previamente "
 ...
 logger.info(f"[INFO] No se encontraron ventas previamente enviadas")
 ...
 logger.warning(f"[WARNING] RESETEANDO tracking de ventas enviadas")
 self.sent_sales = set()
 self._save_tracking()