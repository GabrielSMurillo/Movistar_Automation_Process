...
class OutputFileValidator:
 ...
 return False, [f"[ERROR] Archivo no existe: {file_path.name}"]
 
 # 2. Leer archivo
 try:
 df = pd.read_excel(file_path, sheet_name=0)
 except Exception as e:
 return False, [f"[ERROR] Error leyendo archivo: {e}"]
 
 # 3. Verificar que no esté vacío
 if df.empty:
 errors.append(f"[WARNING] Archivo está vacío")
 ...
 if missing:
 errors.append(f"[ERROR] Faltan columnas: {missing}")
 ...
 errors.append(f"[ERROR] Columna "Linea' no es numérica")
 
 # Verificar que sea secuencial
 if not df['Linea'].is_monotonic_increasing:
 errors.append(f"[ERROR] Columna "Linea' no es secuencial")
 ...
 errors.append(
 f"[WARNING] {count} teléfonos con longitud inválida en '{col}'"
 )
 ...
 errors.append(
 f"[WARNING] {count} teléfonos no numéricos en '{col}'"
 )
 ...
 """
 logger.info("=" * 80)
 logger.info(f"[INFO] VALIDANDO ARCHIVOS DE SALIDA")
 logger.info("=" * 80)
 ...
 logger.info("=" * 80)
 logger.info(f"[INFO] Resumen de Validación:")
