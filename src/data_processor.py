...
from src.validators import PhoneNumberValidator, DataQualityValidator # [ERROR] OLD
# [OK] NEW: Use enhanced phone validator
try:
 ...
 logger.warning(f"[WARNING] EnhancedPhoneValidator not available, using old PhoneNumberValidator")
...
# [OK] NEW: Use correct service code mapper
try:
 ...
 logger.warning(f"[WARNING] ServiceCodeMapper not available, using old get_service_code()")
...
class TipificadorProcessor:
 ...
 Returns:
 (df_ventas, df_referidos, df_invalid, metrics)
 - df_ventas: Ventas válidas de base
 - df_referidos: Ventas referidas
 - df_invalid: Registros inválidos con motivo_rechazo
 - metrics: Métricas del procesamiento
 """
 logger.info("=" * 80)
 logger.info(f"[INFO] PROCESANDO TIPIFICADOR DE VENTAS")
 logger.info("=" * 80)
 ...
 logger.info(f"[INFO] Extrayendo fecha y hora de marca temporal...")
 ...
 logger.info(f"[INFO] Limpiando números telefónicos...")
 
 if ENHANCED_PHONE_VALIDATOR_AVAILABLE:
 # [OK] NEW: Use EnhancedPhoneValidator (handles 957 prefix, complete validation)
 ...
 logger.info(f"[INFO] Using EnhancedPhoneValidator (handles 957 prefix, validates prefixes)")
 else:
 # [ERROR] FALLBACK: Old validator (incomplete)
 ...
 logger.warning(f"[WARNING] Using old PhoneNumberValidator (does not handle 957 prefix)")
 
 # 5. Determinar código de servicio y programa
 logger.info(f"[INFO] Asignando códigos de servicio...")
 
 if SERVICE_MAPPER_AVAILABLE:
 # [OK] NEW: Use ServiceCodeMapper for CORRECT codes per line type
 ...
 logger.info(f"[INFO] Using ServiceCodeMapper (CORRECT codes per MOVIL/FIJA/DIGITAL)")
 else:
 # [ERROR] FALLBACK: Old method (doesn't differentiate MOVIL/FIJA)
 ...
 logger.warning(f"[WARNING] Using old service codes - NOT differentiated by line type!")
 
 # 6. Capturar registros inválidos con motivos de rechazo
 df['motivo_rechazo'] = ''
 df['es_valido'] = True
 
 # Identificar registros inválidos
 invalid_phone = df['telefono_limpio'].isna()
 invalid_date = df['fecha_venta'].isna()
 
 # Asignar motivos de rechazo
 df.loc[invalid_phone & invalid_date, 'motivo_rechazo'] = 'Teléfono y fecha inválidos'
 df.loc[invalid_phone & ~invalid_date, 'motivo_rechazo'] = 'Teléfono inválido'
 df.loc[~invalid_phone & invalid_date, 'motivo_rechazo'] = 'Fecha de venta inválida'
 
 # Marcar como inválidos
 df.loc[invalid_phone | invalid_date, 'es_valido'] = False
 
 # Separar válidos e inválidos
 df_valid = df[df['es_valido']].copy()
 df_invalid = df[~df['es_valido']].copy()
 
 removed_count = len(df_invalid)
 if removed_count > 0:
 logger.warning(
 f"[OK] Identificados {removed_count:,} registros con datos inválidos "
 f"(se generará archivo de NOVEDADES)"
 )
 ...
 logger.info(
 f"[INFO] Separación: {len(df_ventas):,} ventas de base, "
 f"{len(df_referidos):,} referidos"
 )
 
 # 9. Detectar duplicados en ventas de base
 logger.info(f"[INFO] Detectando duplicados en ventas de base...")
 ...
 logger.info(f"[INFO] Validando clasificación de líneas...")
 ...
 logger.info("=" * 80)
 logger.info(f"[INFO] PROCESAMIENTO DEL TIPIFICADOR COMPLETADO")
 logger.info("=" * 80)
 logger.info(f" {} Ventas de base: {len(df_ventas):,}")
 logger.info(f" {} Referidos: {len(df_referidos):,}")
 logger.info(f" {} Rechazados: {len(df_invalid):,}")
 ...
 logger.info(f" {} Fijos: {phone_stats.get('FIJA', 0):,}")
 logger.info("=" * 80)
 
 return df_ventas, df_referidos, df_invalid, metrics
 ...
 logger.error(
 f"[INFO] {len(moviles_incorrectos)} números clasificados como MÓVIL "
 ...
 logger.warning(
 f"[INFO] {len(fijos_incorrectos)} números FIJOS de 10 dígitos "
...
class DigitalProcessor:
 ...
 """
 logger.info("=" * 80)
 logger.info(f"[INFO] PROCESANDO VENTAS DIGITALES")
 logger.info("=" * 80)
 ...
 # 3. Extraer componentes de fecha y hora
 logger.info(f"[INFO] Extrayendo fecha y hora...")
 ...
 # 4. Limpiar teléfonos (Digital: remover prefijo +57)
 logger.info(f"[INFO] Limpiando números telefónicos (removiendo +57)...")
 
 # CRITICAL: Digital sales have +57 prefix (11 digits), remove it
 df['telefono_servicio'] = df['telefono_servicio'].astype(str).str.replace(r'^\+?57', '', regex=True)
 
 if ENHANCED_PHONE_VALIDATOR_AVAILABLE:
 # [OK] NEW: Use EnhancedPhoneValidator
 validator = EnhancedPhoneValidator()
 cleaned_phones, phone_metadata = validator.validate_series(df['telefono_servicio'])
 ...
 logger.info(f"[INFO] Using EnhancedPhoneValidator for digital sales (after +57 removal)")
 else:
 # [ERROR] FALLBACK: Old validator
 cleaned_phones, phone_metadata = PhoneNumberValidator.clean_series(
 df['telefono_servicio'],
 return_metadata=True
 )
 ...
 logger.warning(f"[WARNING] Using old PhoneNumberValidator for digital")
 
 # 5. Determinar código de servicio (versión digital)
 logger.info(f"[INFO] Asignando códigos de servicio (digital)...")
 
 # Para digital, usar ServiceCodeMapper con tipo_linea='DIGITAL'
 if SERVICE_MAPPER_AVAILABLE:
 # [OK] NEW: Use ServiceCodeMapper for digital sales
 mapper = ServiceCodeMapper()
 ...
 logger.info(f"[INFO] Using ServiceCodeMapper for DIGITAL sales")
 else:
 # [ERROR] FALLBACK: Old method
 if 'plan_desc' in df.columns:
 service_info = df['plan_desc'].apply(
 ...
 logger.warning(f"[WARNING] Using old digital service codes")
 ...
 if removed_count > 0:
 logger.warning(f"[OK] Eliminados {removed_count:,} registros inválidos")
 ...
 logger.info("=" * 80)
 logger.info(f"[INFO] PROCESAMIENTO DE VENTAS DIGITALES COMPLETADO")
 logger.info("=" * 80)
...
class HistoricalSalesProcessor:
 ...
 """
 logger.info("=" * 80)
 logger.info(f"[INFO] PROCESANDO VENTAS HISTÓRICAS")
 logger.info("=" * 80)
 
 if df.empty:
 logger.warning(f"[WARNING] No hay ventas históricas para procesar")
 return df, {'historicos': {'total_registros': 0}}
 
 # Validación básica
 if 'telefono_servicio' not in df.columns:
 logger.warning(
 "Columna 'telefono_servicio' no encontrada. Buscando alternativas..."
 )
 ...
 logger.info(f"[INFO] Usando columna '{phone_cols[0]}' como telefono_servicio")
 else:
 logger.error(f"[ERROR] No se encontró columna de teléfono en históricos")
 return df, {'historicos': {'error': 'Sin columna de teléfono'}}
 
 # Estandarizar formato
 if 'telefono_servicio' in df.columns:
 logger.info(f"[INFO] Validando teléfonos históricos...")
 ...
 logger.info(f"[INFO] Clasificando tipo de línea...")
 ...
 logger.info(f"[INFO] Usando columna '{date_cols[0]}' como fecha_venta")
 ...
 logger.info("=" * 80)
 logger.info(f"[INFO] PROCESAMIENTO DE HISTÓRICOS COMPLETADO")
 logger.info("=" * 80)
 logger.info(f" {} Total registros: {len(df):,}")
 if 'tipo_linea' in df.columns:
 logger.info(f" {} Móviles: {tipo_stats.get('MOVIL', 0):,}")
 logger.info(f" {} Fijos: {tipo_stats.get('FIJA', 0):,}")
 logger.info("=" * 80)
...
def consolidate_monthly_report(
 ...
 """
 logger.info("=" * 80)
 logger.info(f"[INFO] CONSOLIDANDO REPORTE MENSUAL")
 logger.info("=" * 80)
 ...
 logger.info(
 f"[INFO] Ventas del mes actual: {len(df_tip_original):,} tipificador + "
 ...
 logger.info(f"[INFO] Total pre-deduplicación: {len(df_consolidated):,} registros")
 ...
 logger.info(f"[INFO] Duplicados eliminados en consolidación: {duplicates_removed:,}")
 logger.info(f"[INFO] Total final: {len(df_final):,} ventas únicas")
 ...
 logger.info("=" * 80)
 logger.info(f"[INFO] CONSOLIDACIÓN COMPLETADA")
 logger.info("=" * 80)
