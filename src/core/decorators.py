...
def timing(func: Callable) -> Callable:
 ...
 logger.info(
 f"[INFO] {func.__name__} completed in {elapsed:.2f}s"
 )
 ...
 elapsed = time.time() - start_time
 logger.error(
 f"[INFO] {func.__name__} failed after {elapsed:.2f}s: {e}"
 )
 raise
 ...
 *,
 ...
 def wrapper(*args, **kwargs) -> Any:
 # Log start
 msg = f"[INFO] Executing {f.__name__}"
 ...
 # Log success
 success_msg = f"[INFO] {f.__name__} completed successfully"
 if include_result:
 ...
 logger.error(f"[ERROR] {f.__name__} raised {type(e).__name__}: {e}")
 raise
 
 return wrapper
 
 # Support both @log_execution and @log_execution()
 if func is None:
 # Called with arguments: @log_execution(level=...)
 return decorator
 else:
 # Called without arguments: @log_execution
...
def cache_result(ttl_seconds: Optional[int] = None):
 ...
 # Never expires
 logger.debug(f"[EMOJI] Cache hit for {func.__name__}")
 return cache[key]
 ...
 logger.debug(
 f"[EMOJI] Cache hit for {func.__name__} "
 f"(age: {elapsed:.1f}s/{ttl_seconds}s)"
 )
 return cache[key]
 else:
 # Expired
 logger.debug(f"[INFO] Cache expired for {func.__name__}")
 del cache[key]
 del cache_timestamps[key]
 
 # Not cached or expired - compute
 logger.debug(f"[INFO] Cache miss for {func.__name__} - computing")
 result = func(*args, **kwargs)
...
def deprecated(reason: str, alternative: Optional[str] = None):
 ...
 @functools.wraps(func)
 def wrapper(*args, **kwargs) -> Any:
 message = f"[INFO] {func.__name__} is deprecated: {reason}"
 if alternative:
