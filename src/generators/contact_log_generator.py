...
class ContactLogGenerator:
 ...
 logger.info("=" * 80)
 logger.info(f"[INFO] GENERANDO CONTACT LOG")
 logger.info("=" * 80)
 
 if df.empty:
 logger.warning(f"[WARNING] DataFrame vacío, no se genera Contact Log")
 return False
 ...
 logger.info(
 f"[INFO] Ventas válidas para Contact Log: {len(df_valid):,} / {len(df):,}"
 )
 
 if df_valid.empty:
 logger.warning(f"[WARNING] No hay ventas válidas después del filtrado")
 return False
 
 # 2. Construir registros
 contact_records = self._build_contact_records(df_valid)
 logger.info(f"[INFO] Registros construidos: {len(contact_records):,}")
 ...
 logger.error(f"[INFO] Validación de Contact Log falló")
 return False
 
 # 7. Log de éxito
 logger.info("=" * 80)
 logger.info(f"[INFO] CONTACT LOG GENERADO EXITOSAMENTE")
 logger.info("=" * 80)
 logger.info(f" {} Archivo: {output_path.name}")
 logger.info(f" {} Total registros: {len(df_contact):,}")
 logger.info(f" [OK] Procesados: {self.records_processed:,}")
 logger.info(f" ⊘ Omitidos: {self.records_skipped:,}")
 logger.info(f" [EMOJI] Tamaño: {output_path.stat().st_size / 1024:.2f} KB")
 logger.info("=" * 80)
 ...
 if removed > 0:
 logger.info(f" {} Registros filtrados: {removed:,}")
 ...
 logger.warning(
 f"[ERROR] Error procesando registro (índice {idx}): {e}"
 )
 ...
 if pd.isna(hora_venta) or hora_venta == '':
 hora = 'N/A'
 elif isinstance(hora_venta, str):
 # La hora viene en formato "HH:MM:SS AM/PM" desde extract_datetime_components
 # o puede venir como timestamp completo "YYYY-MM-DD HH:MM:SS"
 hora_str = str(hora_venta).strip()
 
 # Si tiene fecha incluida (contiene '-'), extraer solo la parte de hora
 if '-' in hora_str and ' ' in hora_str:
 # Formato: "2025-10-14 10:25:00" -> extraer "10:25:00"
 hora = hora_str.split(' ')[1]
 else:
 # Ya es solo hora: "10:25:00 AM" o "10:25:00"
 hora = hora_str
 else:
 hora = str(hora_venta)
 ...
 IOError: Si hay error al guardar el archivo
 """
 logger.debug(f"[EMOJI] Guardando Excel: {output_path}")
 ...
 logger.debug(f" [OK] Excel guardado exitosamente")
 ...
 """
 logger.debug(f"[INFO] Validando Contact Log: {file_path.name}")
 
 # 1. Verificar existencia
 if not file_path.exists():
 logger.error(f"[ERROR] Archivo no existe: {file_path}")
 return False
 ...
 if len(df.columns) != 3:
 logger.error(
 f"[INFO] Número incorrecto de columnas: "
 ...
 if df.empty:
 logger.warning(f"[WARNING] Archivo vacío (pero válido estructuralmente)")
 return True
 ...
 logger.warning(
 f"[INFO] {len(invalid_phones)} teléfonos con formato inválido"
 )
 ...
 if empty_obs.any():
 logger.warning(
 f"[WARNING] {empty_obs.sum()} registros con Campo Observacion vacío"
 )
 ...
 if empty_razon.any():
 logger.warning(
 f"[WARNING] {empty_razon.sum()} registros con Campo Razon vacío"
 )
 
 logger.debug(f"[INFO] Validación de Contact Log: APROBADA")
 return True
 
 except Exception as e:
 logger.error(f"[INFO] Error validando Contact Log: {e}", exc_info=True)
 return False
...
def generate_quality_reports(
 ...
 """
 logger.info("\n" + "=" * 80)
 logger.info(f"[INFO] GENERANDO REPORTES DE CALIDAD")
 logger.info("=" * 80)
 ...
 logger.info(f"[INFO] Reporte de calidad generado: {report_path.name}")
 logger.info("=" * 80)
