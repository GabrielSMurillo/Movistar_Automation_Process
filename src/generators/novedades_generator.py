...
class NovedadesGenerator(BaseGenerator):
 ...
 self.logger.info("=" * 80)
 self.logger.info(f"[PROCESO] GENERATING NOVEDADES FILE")
 self.logger.info("=" * 80)
 
 # Check if there are rejected records
 if df.empty:
 self.logger.info(f"[INFO] No hay registros rechazados - archivo de novedades no necesario")
 return True
 ...
 self.logger.info(f"[INFO] Novedades: {len(df_novedades):,} registros rechazados")
 ...
 """Log rejection statistics."""
 self.logger.info("\n" + "-" * 80)
 self.logger.info(f"[INFO] ESTADÍSTICAS DE RECHAZO")
 self.logger.info("-" * 80)
 
 self.logger.info(f"[INFO] Total registros rechazados: {len(df):,}")
 
 # Count by validation type (solo si existen las columnas)
 validation_columns = {
 'VALIDACION_TELEFONO': f'[ERROR] Errores de teléfono',
 'VALIDACION_ASESOR': '[EMOJI] Errores de asesor',
 'VALIDACION_LOGIN': '[EMOJI] Errores de login',
 'VALIDACION_CLIENTE': '[EMOJI] Errores de cliente'
 }
 
 for col, label in validation_columns.items():
 if col in df.columns:
 try:
 errors = int((~df[col]).sum())
 self.logger.info(f"{label}: {errors:,}")
 except Exception:
 pass # Skip si hay error
