...
Architecture:
 BaseGenerator (ABC)
 [EMOJI] ContactLogGenerator
 [EMOJI] FormatoMovistarGenerator
 [EMOJI] SVASGenerator
 [EMOJI] MonthlyReportGenerator
...
class BaseGenerator(ABC):
 ...
 self.logger.info("=" * 80)
 self.logger.info(f"[EXITO] {self.file_type.upper()} GENERATION SUCCESSFUL")
 self.logger.info("=" * 80)
 ...
 self.logger.info(f" [EMOJI] File size: {file_size:.2f} KB")
 if self.generation_time:
 self.logger.info(f" {} Generation time: {self.generation_time:.2f}s")
 self.logger.info("=" * 80)
 else:
 self.logger.error("=" * 80)
 self.logger.error(f"[ERROR] {self.file_type.upper()} GENERATION FAILED")
 self.logger.error("=" * 80)
