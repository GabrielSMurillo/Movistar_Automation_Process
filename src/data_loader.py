...
try:
    ...
    # Fallback si no estan disponibles
...
class CSVLoader:
    ...
    def load_csv(
        ...
        if not file_path.exists():
            raise FileNotFoundError(
                ...
                f"Asegurate de exportar el archivo desde Google Sheets como CSV"
            )
        
        # Combinar configuracion
        ...
        logger.info(f"[INPUT] Cargando CSV: {file_path.name}")
        logger.debug(f"Configuracion de lectura: {config}")
        ...
        try:
            ...
            logger.info(
                f"[OK] Archivo cargado: {len(df):,} filas, {len(df.columns)} columnas"
            )
            ...
            if not is_valid:
                logger.error(f"[WARNING] Problemas de calidad: {message}")
            
            return df
        
        except pd.errors.ParserError as e:
            logger.error(
                f"[ERROR] Error parseando CSV '{file_path.name}': {e}\n"
                f"Verifica que el archivo este correctamente exportado desde Sheets"
            )
            raise
        
        except Exception as e:
            logger.error(f"[ERROR] Error inesperado cargando '{file_path.name}': {e}")
            raise
    
    @staticmethod
    def _clean_dataframe(df: pd.DataFrame) -> pd.DataFrame:
        """Limpieza automatica de datos comunes en CSVs de Sheets."""
        
        # 1. Eliminar columnas completamente vacias
    ...
    def load_and_concatenate_csvs(
        ...
        if not matching_files:
            logger.warning(
                f"No se encontraron archivos con patron '{pattern}' en {directory}"
            )
            return pd.DataFrame()
        
        logger.info(f"[FOLDER] Encontrados {len(matching_files)} archivos CSV para concatenar")
        ...
        for file_path in matching_files:
            try:
                ...
                logger.error(f"[WARNING] Error cargando '{file_path.name}': {e}. Omitiendo.")
                continue
        
        if not df_list:
            logger.error(f"[ERROR] No se pudo cargar ningun archivo correctamente")
            return pd.DataFrame()
        ...
        logger.info(
            f"[OK] Concatenados {len(df_list)} archivos: "
...
def load_tipificador(config: dict) -> pd.DataFrame:
    ...
    Ahora soporta CSV y Excel automaticamente.
    """
    file_path = Path(config['file_path'])
    skiprows = config.get('skiprows', 0)
    
    # Si es CSV, usar el metodo tradicional
    if file_path.suffix.lower() == '.csv':
        return CSVLoader.load_csv(file_path, skiprows=skiprows)
    
    # Si es Excel, usar InputAdapter
    logger.info(f"[INPUT] Cargando Tipificador (Excel): {file_path.name}")
    adapter = InputAdapterFactory.create(file_path, skiprows=skiprows)
    df = adapter.read()
    df = CSVLoader._clean_dataframe(df)
    logger.info(f"[OK] Archivo cargado: {len(df):,} filas, {len(df.columns)} columnas")
    return df
...
def load_digital(config: dict) -> pd.DataFrame:
    ...
    # Si es Excel, usar InputAdapter
    logger.info(f"[INPUT] Cargando Digital (Excel): {file_path.name}")
    adapter = InputAdapterFactory.create(file_path, skiprows=skiprows)
    df = adapter.read()
    df = CSVLoader._clean_dataframe(df)
    logger.info(f"[OK] Archivo cargado: {len(df):,} filas, {len(df.columns)} columnas")
