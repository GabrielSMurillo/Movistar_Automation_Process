...
class EnhancedPhoneValidator:
 ...
 self.logger.info("=" * 60)
 self.logger.info(f"[INFO] VALIDACIÓN DE TELÉFONOS")
 self.logger.info("=" * 60)
 self.logger.info(f"Total procesados: {len(series):,}")
 self.logger.info(f"[OK] Válidos: {valid_count:,} ({valid_count/len(series)*100:.1f}%)")
 self.logger.info(f"[OK] Inválidos: {invalid_count:,} ({invalid_count/len(series)*100:.1f}%)")
 self.logger.info(f"[INFO] Móviles: {movil_count:,}")
 self.logger.info(f"[INFO] Fijos: {fija_count:,}")
