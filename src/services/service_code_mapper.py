...
class ServiceCodeMapper:
 ...
 'TU MASCOTA': ['MASCOTA', 'MASCOTAS', 'PET', 'CANITAS', 'CANITA'],
 'TU HOGAR': ['HOGAR', 'CASA', 'HOME'],
 'TU VEHICULO': ['VEHICULO', 'VEHÍCULO', 'AUTO', 'CARRO', 'VEHICLE', 'MOTOCICLETA', 'MOTO'],
 }
 ...
 # Skip invalid records (they will be filtered out later)
 if tipo_linea_upper == 'INVALIDO' or tipo_linea_upper not in ['DIGITAL', 'MOVIL', 'FIJA']:
 self.logger.debug(f"Skipping mapping for invalid line type: {tipo_linea}")
 return ('0000', 'INVALIDO')
 
 if tipo_linea_upper == 'DIGITAL':
 # Digital products have different names, don't normalize
 return self._get_digital_code(tipo_venta_upper)
 elif tipo_linea_upper == 'MOVIL':
 # Normalize product type for MOVIL/FIJA
