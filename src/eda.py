...
class EDAGenerator:
 ...
 """
 logger.info(f"[INICIO] Iniciando EDA para: {df_name}")
 
 if df.empty:
 logger.warning(f"[WARNING] DataFrame '{df_name}' vacío. Omitiendo EDA.")
 return
 
 output_dir.mkdir(parents=True, exist_ok=True)
 
 # 1. Reporte de calidad
 logger.info(f" {} Generando reporte de calidad...")
 quality_report = DataQualityValidator.generate_quality_report(df, df_name)
 
 # 2. Estadísticas generales
 logger.info(f" {} Generando estadísticas descriptivas...")
 stats_report = EDAGenerator._generate_stats_report(df, df_name)
 
 # 3. Análisis de valores categóricos
 logger.info(f" {} Analizando variables categóricas...")
 ...
 logger.info(f"[EXITO] EDA completado para: {df_name}")
