...
def standardize_date(
    ...
    if invalid_count > 0:
        logger.warning(
            f"⚠️ {invalid_count:,} fechas inválidas "
...
def find_duplicates(
    ...
    logger.info(f"🔍 Buscando duplicados basados en: {subset_cols}")
    ...
    if duplicate_count > 0:
        logger.warning(
            f"⚠️ Encontrados {duplicate_count:,} duplicados "
...
def find_empaquetados(
    ...
    logger.info(f"🔍 Identificando ventas empaquetadas...")
    ...
    if empaquetado_count > 0:
        logger.info(
            f"📦 Encontradas {empaquetado_count:,} ventas empaquetadas "
...
def validate_columns(
    ...
    if missing_cols:
        error_msg = (
            f"❌ Faltan columnas requeridas en '{df_name}':\n"
...
def detect_referidos(
    ...
    logger.info(
        f"🔖 Identificados {referido_count:,} referidos "
        f"({referido_count/len(df)*100:.1f}%)"
    )
