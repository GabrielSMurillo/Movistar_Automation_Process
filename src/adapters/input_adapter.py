 ...
 **kwargs: Argumentos adicionales para pandas.read_csv
 ...
 """
 try:
 logger.debug(f"[INFO] Leyendo CSV: {self.file_path.name}")
 ...
 **self.kwargs
 ...
 logger.debug(f" [OK] Leídas {len(df):,} filas, {len(df.columns)} columnas")
 
 return df
 
 except Exception as e:
 logger.error(f"[ERROR] Error leyendo CSV {self.file_path.name}: {e}")
 ...
 **kwargs: Argumentos adicionales para pandas.read_excel
 ...
 """
 try:
 logger.debug(f"[INFO] Leyendo Excel: {self.file_path.name}")
 ...
 **self.kwargs
 ...
 logger.error(f"[ERROR] Error leyendo Excel {self.file_path.name}: {e}")
 ...
 logger.error(f"[ERROR] Error listando hojas: {e}")
 return []
 ...
 **kwargs: Argumentos adicionales para el adapter
 ...
 logger.debug(f"[EMOJI] Factory: Detectando adapter para {file_path.name}")
 logger.debug(f" Extensión: {extension}")
 
 # Determinar adapter según extensión
 if extension == '.csv':
 logger.debug(" [OK] Seleccionado: CSVAdapter")
 return CSVAdapter(file_path, **kwargs)
 
 elif extension in ['.xlsx', '.xls', '.xlsm']:
 logger.debug(" [OK] Seleccionado: ExcelAdapter")
